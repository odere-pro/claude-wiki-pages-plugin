...
- **Email**: `odere.pro@gmail.com` with subject prefix `[security][claude-wiki-pages]`.
...
**Residual check→write window on the durable-memory carve-out (C13 / Architect ruling — intentional).**
`protect-raw.sh` is a `PreToolUse` advisory hook: it inspects `.tool_input.content` and the (not-yet-existing) target path, emits a `block`/allow decision, and the harness performs the actual `Write` afterward. The new-file test (`[ ! -f "${CANONICAL_PATH}" ]`) and the subsequent write are therefore not under the cross-process `scripts/vault-lock.sh` flock — a TOCTOU window exists in principle. This is **not closed with a lock**, by design: the hook does not perform the write, so it cannot hold a lock across an operation it does not control, and bolting one on would fight the PreToolUse architecture without closing the window. The window is bounded structurally instead: the carve-out is new-file-only and any overwrite of an existing `raw/` file is unconditionally blocked (`scripts/protect-raw.sh:121-124`), so two racing writes to the same `agent-sessions/` path collapse to the immutable-overwrite block; the engine's own writes target `wiki/`/`_proposed/`, never `raw/agent-sessions/` (ADR-0010 §6, §"Negative"). Raw immutability is not weakened by accepting this residual window. See ADR-0010 for the carve-out's full fail-closed envelope.

...
| ----------------------- | ----------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------- |
| Layer 1 — Data          | `skills/init/template/`                                 | Immutable `raw/`, LLM-maintained `wiki/`, schema in `skills/init/template/CLAUDE.md`. Passive.                                                                                                                                                                                                                                                                                                                                                         | §4, §6, §7, §8 |
| Layer 2 — Skills        | `skills/`                                             | 26 single-responsibility capabilities: 14 plugin-authored short verbs (`init`, `ingest`, `query`, `lint`, `fix`, `status`, `synthesize`, `index`, `markdown`, `search`, `review`, `draft`, `sync`, `fill-gaps`), `onboarding`, 6 agent-teaching skills (`engine-api`, `maintain-contract`, `analyst-modes`, `curator-fixes`, `ingest-pipeline`, `voice`), `obsidian-graph-colors`, `obsidian-vault`, plus 3 MIT-licensed `obsidian-*` reference skills (`kepano/obsidian-skills`). | §5, §9         |
| Layer 3 — Agents        | `agents/`                                             | 8 multi-step executors: `claude-wiki-pages-orchestrator-agent` (top-level entry), `claude-wiki-pages-onboarding-agent`, `claude-wiki-pages-ingest-agent`, `claude-wiki-pages-extract-worker-agent`, `claude-wiki-pages-curator-agent`, `claude-wiki-pages-analyst-agent`, `claude-wiki-pages-polish-agent`, `claude-wiki-pages-maintenance-agent`.                                                                                                                                             | §5, §11        |
| Layer 4 — Orchestration | `commands/`, `hooks/hooks.json`, `scripts/`, `rules/` | `/claude-wiki-pages:wiki`, `/claude-wiki-pages:onboarding`, `/claude-wiki-pages:doctor`, and `/claude-wiki-pages:fill-gaps` slash commands; `SessionStart`, `UserPromptSubmit`, `PreToolUse`, `PostToolUse`, `SubagentStop` hooks; script implementations; path-scoped rules.                                                                                                                                                                                                        | §5, §9, §10    |
...
| Frontmatter            | `skills/init/template/CLAUDE.md`; `skills/init/template/_templates/`                                                                                                                                                                          |
| User-facing prose      | `docs/GLOSSARY.md`; `docs/llm-wiki/` for voice                                                                                                                                                                                            |
| Graph shape            | ADR-0033 (topic-local linking + topic-island view + root entry-point node); `scripts/disentangle-links.sh` (remediation), `scripts/graph-quality.sh` (metric); `skills/obsidian-graph-colors`                                            |
| Context + interop      | `engine context --skill <name>` (L0–L4 ICM context resolution, `src/commands/context/`); `engine okf export\|import` (OKF round-trip, `src/commands/okf/`); `scripts/scope-guard.sh` (read-scope advisory hook)                          |
| Research foundations   | `docs/research-foundations.md` (prior art: Karpathy LLM Wiki, NO-RAG vs RAG, ICM, OKF, PROV, Porter, force-directed layout)                                                                                                              |
...
| Local models / offline | `docs/local-models.md` (capability tiers, per-tier allow-list `APPROVED_LOCAL_MODELS_BY_TIER`, tested models); ADR-0011/0017/0018/0019; `scripts/offline-draft.sh`, `scripts/offline-query.sh`, `scripts/reachability.sh`, engine `route` |

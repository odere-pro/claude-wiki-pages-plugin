...
// "left"/"remaining": run the whole tail of the plan in one go — docs (Phase 1
// item 9) FIRST, then Phase 2 concurrency, then Phase 3 security gates. Stop the
// chain if any phase halts red (do not build later phases on a red foundation).
async function runRemaining() {
  const docsUnit = PHASE1.find((u) => u.id === 'docs')
  const out = { mode: 'left', sequence: ['docs', 'phase2', 'phase3'] }

  log('=== Phase 1 item 9 — docs: validate-docs.sh → lint --check docs (fan-out) ===')
  out.docs = await runFanOut([docsUnit])
  if (halted(out.docs)) {
    log('HALT after docs — Phase 2/3 NOT started. Review the red docs unit (ADR-0013 Check 5 / 0–4 divergence are the known blockers).')
    return out
  }
  ...
  if (halted(out.phase2)) {
    log('HALT after Phase 2 — Phase 3 NOT started.')
    return out
  }

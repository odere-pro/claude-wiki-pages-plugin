...
| layer coloring          | An optional graph-color pass (`path:raw`→green, `path:wiki`→blue, `path:_templates`→orange) ordered after per-topic colors and the `_sources`/`_synthesis` specials, so the three-layer structure is visible at a glance. Applied by the polish agent via `obsidian-graph-colors`.                |

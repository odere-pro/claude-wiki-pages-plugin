...
A page under `vault/_proposed/` with `status: draft` and `proposed_by`. Mirrors its eventual `wiki/` path; outside every wiki-scoped check until promoted.

## Key Principles

- A page under `vault/_proposed/` with `status: draft` and `proposed_by`.

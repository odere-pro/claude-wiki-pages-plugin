...
Vault isolation: the PreToolUse boundary (`scripts/firewall.sh` + the engine `firewall` command) that confines agent writes to the resolved vault plus `allowPaths`, minus `denyPaths`. Modes: `enforce`/`warn`/`off`.

## Key Principles

- Vault isolation: the PreToolUse boundary (`scripts/firewall.sh` + the engine `firewall` command) that confines agent writes to the resolved vault plus `allowPaths`, minus `denyPaths`.

...
The tail-of-write step (`claude-wiki-pages-polish-agent`) that keeps the Obsidian-side experience in sync — graph colors, vault MOC, per-folder MOC consistency. Runs after every ingest or curator.

## Key Principles

- The tail-of-write step (`claude-wiki-pages-polish-agent`) that keeps the Obsidian-side experience in sync — graph colors, vault MOC, per-folder MOC consistency.

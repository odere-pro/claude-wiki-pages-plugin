...
The `SubagentStop` safety net (`scripts/subagent-commit-gate.sh`): after a write-path agent returns, any vault changes left uncommitted are committed as one labelled backstop commit. Pathspec-scoped to the vault; never blocks.

## Key Principles

- The `SubagentStop` safety net (`scripts/subagent-commit-gate.sh`): after a write-path agent returns, any vault changes left uncommitted are committed as one labelled backstop commit.
...
- `SubagentStop`
- `scripts/subagent-commit-gate.sh`

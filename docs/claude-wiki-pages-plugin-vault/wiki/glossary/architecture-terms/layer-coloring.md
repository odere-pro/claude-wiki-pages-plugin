...
**Dropped** (ADR-0023). The former graph-color pass (`path:raw`→green, `path:wiki`→blue, `path:_templates`→orange) that colored non-wiki content. Superseded by the wiki-only graph: excluded paths never reach the graph, so layer groups are dead weight.

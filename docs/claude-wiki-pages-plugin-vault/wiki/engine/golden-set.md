...
related: ["[[local-model-quality-gate|Local Model Quality Gate]]", "[[zero-fabrication-floor|Zero-Fabrication Floor]]", "[[approved-local-model|Approved Local Model]]", "[[capability-tier|Capability Tier]]", "[[scaffolding-ablation|Scaffolding Ablation]]", "[[schema-authority|Schema Authority]]", "[[parity-gate|Parity Gate]]"]
contradicts: []
supersedes: []
depends_on: []
...
- The same golden set is the common substrate for both the [[local-model-quality-gate|Local Model Quality Gate]] and the [[scaffolding-ablation|Scaffolding Ablation]], making the two studies directly comparable.
...
The golden set lives in `tests/eval/ingest-extract/` and serves as the evaluation corpus for the [[local-model-quality-gate|Local Model Quality Gate]] (ADR-0011). It is:
...
The golden set includes at least one fixture whose raw input is authored so that a fabricating model is forced to invent a sourced claim in order to produce a plausible answer. This exercises the [[zero-fabrication-floor|Zero-Fabrication Floor]] directly — if a model trips the trap, it invented a claim, and the floor fires. If it does not trip the trap, the floor is satisfied.
...
The [[scaffolding-ablation|Scaffolding Ablation]] (ADR-0020) uses the same golden set as its measured input. It runs two arms — plugin arm (full scaffolding) and baseline arm (generic prompt) — against the same golden inputs to measure what the plugin buys over plain LLM extraction. The golden set is the common substrate for both the quality gate and the ablation study.

## Related Concepts

- [[local-model-quality-gate|Local Model Quality Gate]] — the gate that uses the golden set for tier evaluation
- [[zero-fabrication-floor|Zero-Fabrication Floor]] — the hard floor exercised by the provenance trap
- [[approved-local-model|Approved Local Model]] — the allow-list result of a model passing all golden-set thresholds
- [[capability-tier|Capability Tier]] — the tier a model is evaluated against
- [[scaffolding-ablation|Scaffolding Ablation]] — the companion eval that uses the same inputs to measure plugin value

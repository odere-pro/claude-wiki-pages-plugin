...
related: ["[[analyst-agent|Analyst Agent]]", "[[query-rules|Query Rules]]", "[[synthesis-note|Synthesis Note]]", "[[architecture-decision-record|Architecture Decision Record]]", "[[no-rag-principle|NO-RAG Principle]]"]
...
Challenge mode is not a configuration flag or a separate agent — it is an invocation pattern for the [[analyst-agent|Analyst Agent]]. The pattern is that instead of asking "what does the wiki say about X?", the user frames the question adversarially: "I'm about to decide X — push back on my assumptions."
...
1. **`contradicts:` relationships.** Pages that explicitly contradict the topic in question. If page A has a `contradicts:` entry pointing to a page related to the decision, the analyst surfaces it.
...
The analyst reads [[local-model-quality-gate|Local Model Quality Gate]], [[approved-local-model|Approved Local Model]], [[capability-tier|Capability Tier]], and the decisions folder. It surfaces ADR-0011's measured rejections table, the governance rule that each tier needs its own golden set and ADR amendment before unlocking, and any low-confidence claims about local model synthesis quality.
...
- [[analyst-agent|Analyst Agent]] — the agent with Challenge as one of its five modes
- [[query-rules|Query Rules]] — the broader query workflow the analyst follows
- [[synthesis-note|Synthesis Note]] — the analyst may offer to file the challenge result as a synthesis

...
> [!summary]
> The `claude-wiki-pages-analyst-agent` answers questions from the wiki in five distinct operating modes: Query, Dashboard, Document Compile, Extract, and Challenge. It is dispatched by the [[orchestrator-agent|Orchestrator Agent]] when the user's prompt carries an analytical verb. Every answer ends with a `## Sources` section — wikilink citations in research-paper style. The agent reads but never writes the wiki.
...
- Dispatched by: [[orchestrator-agent|Orchestrator Agent]] when the user prompt contains an analytical verb (`what`, `why`, `compare`, `how`, `which`, `summarize`)
...
- **Defer on write requests:** if the user asks the analyst to add a page or update a source, the agent routes back to the [[orchestrator-agent|Orchestrator Agent]] for the ingest path rather than writing directly.

## What the Agent Defers

The analyst does not make judgments about wiki structure (orphans, index drift, stale pages) — that is the [[curator-agent|Curator Agent]]'s domain. It does not ingest new raw sources — that is the [[ingest-agent|Ingest Agent]]'s domain. It does not regenerate graph colors or folder notes — that is the [[polish-agent|Polish Agent]]'s domain.

...
The [[llm/software-3-0|Software 3.0]] posture (ADR-0013) states that every project surface must be equally usable by humans and agents. A surface with a human form but no agent form is incomplete — an agent cannot invoke it. A surface with an agent form but no human form lacks documentation for the human who needs to understand, audit, or debug it.

...
related: ["[[parity-gate|Parity Gate]]", "[[node-grounding|Node Grounding]]", "[[design-drift-gate|Design-Drift Gate]]", "[[four-layer-stack|Four-Layer Stack]]"]
contradicts: []
supersedes: []
depends_on: []
...
> [!summary]
> Software 3.0 is the design posture of the claude-wiki-pages project: every project surface — every command, every script, every API endpoint — must be equally usable by humans and autonomous agents. The posture is embodied in the dual-entry router table and enforced by the [[parity-gate|Parity Gate]] in CI. It motivates the four-layer stack's separation of concerns and the preference for deterministic, auditable mechanisms over implicit or model-dependent ones.
...
- The [[parity-gate|Parity Gate]] in CI enforces the dual-entry table — an asymmetric surface fails CI.
...
A row with an empty "Agent form" column fails the [[parity-gate|Parity Gate]] in CI.
...
**Dual-entry router table.** Design documents contain a table mapping each surface to its human form and its agent form. The [[parity-gate|Parity Gate]] enforces that neither column is empty. Example pairs: `/claude-wiki-pages:wiki` (human) ↔ `claude-wiki-pages-ingest-agent` (agent), `engine.sh verify` (both).

**Bash-first Layer 4.** The scripts layer is shell-first so hooks are available without Bun. A human debugging a broken install can read and run the same bash scripts the hooks run. [[shell-ts-parity|Shell-TS Parity]] ensures the TypeScript twin agrees with the bash twin.
...
- [[parity-gate|Parity Gate]] — the CI gate that enforces human-agent parity in the dual-entry router table
- [[node-grounding|Node Grounding]] — another Design-Drift Gate check that enforces diagram accuracy for both human readers and agents
- [[design-drift-gate|Design-Drift Gate]] — the overall gate that enforces Software 3.0 design discipline
- [[four-layer-stack|Four-Layer Stack]] — the architecture whose structure embodies the Software 3.0 posture

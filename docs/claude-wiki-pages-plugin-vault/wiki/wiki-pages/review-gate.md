...
related: ["[[durable-memory|Durable Memory]]", "[[draft-review-surface|Draft Review Surface]]", "[[ingest-agent|Ingest Agent]]", "[[curator-agent|Curator Agent]]", "[[Firewall]]"]
contradicts: []
supersedes: []
depends_on: []
...
The review gate is managed by the [[draft-review-surface|Draft Review Surface]] (engine `review`, `approve`, `reject` verbs):
...
The [[durable-memory|Durable Memory]] write-back workflow is designed for low overhead (a session writes learnings, the next wiki call queues them for review) not zero overhead. The human review step is intentional.

## Related Concepts

- [[durable-memory|Durable Memory]] — the mechanism that produces agent-session drafts through the review gate
- [[draft-review-surface|Draft Review Surface]] — the engine verbs (`review`, `approve`, `reject`) that manage the gate
- [[ingest-agent|Ingest Agent]] — processes `raw/agent-sessions/` sources and creates drafts in `_proposed/`
- [[curator-agent|Curator Agent]] — runs after approval to heal the wiki and update indexes

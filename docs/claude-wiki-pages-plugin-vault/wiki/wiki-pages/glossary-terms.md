...
> [!summary] > `docs/GLOSSARY.md` is the canonical term list for `claude-wiki-pages`. Every doc, skill, and user-visible string must conform to it. `validate-docs.sh` enforces it in CI Tier 0. The glossary is "input weight" — using established community terms activates LLM priors and lowers session drift. It has two registers: Technical (inside the product) and Discoverability (SEO surfaces only).

## Key Principles

- The glossary is "input weight" — established community terms (MOC, vault, wiki, frontmatter, provenance, ingest) activate LLM priors and lower session drift; novel compounds waste weight.
- Two registers, never mixed: Technical (inside the product) and Discoverability (SEO surfaces only); `validate-docs.sh` flags discoverability terms in technical surfaces.
- Enforcement runs in CI Tier 0 (bash/grep only); violations produce `path:line:column rule-id description` output.
- The glossary is semver: additions are minor bumps; renames or meaning changes are major bumps.
- Content inside fenced code blocks and inline code spans is exempt from the scan — only prose heading text and body text are checked.

## Examples

Running the glossary gate locally:

```bash
bash scripts/validate-docs.sh
# Check 1 — glossary gate: scans docs/, skills/, agents/ for banned strings and register violations
# Exit 0 = clean; Exit 1 = violation(s) listed as path:line:column rule-id description
```

Banned terms since the 1.0.0 rebrand:

```
llm-wiki-stack               → claude-wiki-pages
/llm-wiki-stack:wiki         → /claude-wiki-pages:wiki
llm-wiki-stack-ingest-agent  → claude-wiki-pages-ingest-agent
```

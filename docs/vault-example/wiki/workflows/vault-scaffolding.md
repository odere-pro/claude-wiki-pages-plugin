...
The one-time workflow for standing up a new `vault/` in a project. Invoked as `/claude-wiki-pages:init`. Copies an empty starter scaffold into the project, writes the per-vault `CLAUDE.md`, and persists the chosen vault path into `.claude/claude-wiki-pages/settings.json` so subsequent sessions resolve it automatically.
...
- Fresh project: `/claude-wiki-pages:init` — scaffolds at the default path `docs/vault`.
- Custom path: `/claude-wiki-pages:init my vault is docs/my-research`.

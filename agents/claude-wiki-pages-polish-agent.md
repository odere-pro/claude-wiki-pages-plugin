...
5. Assert the **wiki-only exclusions and Obsidian write-protection keys** (per the `obsidian-graph-colors` skill): ensure `vault/.obsidian/app.json` has a `userIgnoreFilters` array containing `raw/`, `_templates/`, `_proposed/`, and `_inbox/`, plus the three new-file keys `newFileLocation: "folder"`, `newFileFolderPath: "_inbox"`, and `newLinkFormat: "shortest"`. Merge-only: create the file if absent; append the missing `_inbox/` entry to `userIgnoreFilters`; set each of the three keys only when it is absent or differs; preserve every other key; never remove user entries. These keys quarantine Obsidian-created stub notes into the excluded `_inbox/` folder so they never shadow a real wiki page at the vault root (an exact-filename match beats an alias in Obsidian). Never add color groups for the excluded paths — the graph shows only generated wiki pages.

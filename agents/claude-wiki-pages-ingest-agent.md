...
For each source, write to `vault/wiki/_sources/<kebab-slug>.md` using the `source` frontmatter schema from `vault/CLAUDE.md`. Body: Metadata, Summary, Key Claims (prose) — nothing else.

**The source summary carries NO outbound `[[wikilinks]]`.** Do not emit `## Entities Mentioned`, `## Concepts Mentioned` / `## Concepts Covered`, or `## Grounded Pages` sections — those out-links fan across every topic a source touches and collapse the graph's topic islands into a hairball. Provenance is one-directional: **page → source**. Instead of linking out from the source, ensure every wiki page this source informs lists it in that page's own `sources:` frontmatter (Steps 1.5 and 1.7 already do this — `sources` is non-negotiable on every typed page). A source is then reached by the pages that cite it, so it clusters with its topic rather than bridging all of them. The piped-basename `sources:` citation resolves in Obsidian, so this inbound path is real — the old workaround of out-linking from the source (added because bare-alias citations did not resolve) is obsolete and is removed. If you want to record what a source covers, add a single PLAIN-TEXT line under Key Claims — `Covers: Specialist Pattern, Orchestrator, Ingest Pipeline` (names only, no `[[ ]]`).

> [!important] Each source must still earn ≥1 inbound citation (no orphan-source)
> Because the source no longer self-links, its only inbound edges come from the pages that cite it. When you create or update pages in Step 1.5, make every page the source informed add this source to its `sources:` field. Verify before finishing the run that each summary written here is cited by at least one wiki page; an uncited source is an orphan and a lint finding.
...
- **All internal references use piped basename `[[wikilinks]]`** — target the destination's file basename with a Title-Case display, `[[page-basename|Page Title]]`, in body text and in every frontmatter link field (`parent`, `sources`, `related`, `children`, `child_indexes`, `key_pages`, `members`, `scope`, `depends_on`). Obsidian resolves a written link by exact vault path or basename only — never by `aliases:`/`title:`, so a bare `[[Page Title]]` does not resolve. **Default to the bare basename form `[[page-basename|Display]]`.** Path-qualify to `[[topic/page-basename|Display]]` **only when that basename occurs in 2+ files anywhere in the vault** (a genuine collision — e.g. a `_sources/X.md` summary that collides with its `raw/.../X.md` original). When you path-qualify, never guess the folder: use the target page's **actual** wiki-relative path and verify the page exists at that path — over-qualifying a unique basename with a guessed folder (e.g. `[[how-it-works/ingest-pipeline]]` for a page that lives in `wiki-pages/`) creates a dangling link. Never use `[text](path.md)`.

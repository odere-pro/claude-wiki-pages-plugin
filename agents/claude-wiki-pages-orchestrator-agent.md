...
3. Add a one-paragraph summary under `## Outcome`: what changed in `$VAULT`, what the user should know, and (if applicable) the suggested next `/claude-wiki-pages:wiki` invocation. Always frame the next step as `/claude-wiki-pages:wiki` — it is the one advertised entry verb.

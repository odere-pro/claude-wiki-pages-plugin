...
| `graph_health`      | `bash ${CLAUDE_PLUGIN_ROOT}/scripts/health-score.sh --target "$VAULT" --json` → cache `.score`, `.grade`, `.needsHeal`, and `.issues[]`. Read-only; the single detect signal for graph/config drift (dangling, orphans, non-spine/cross-tree edges, low tree conformance, stale `.obsidian`, ghost links). Skipped only when `vault_exists` is false.                       | {score, grade, needsHeal, issues} |

...
Feature requests are triaged into `v0.2`, `v1.0`, or `wontfix` labels. `wontfix` always carries a one-line rationale (typically: "out of scope per the project [non-goals in the README](./README.md)").
...
- The vault `schema_version` and the plugin `version` are independent. A schema bump never lands without an explicit `CHANGELOG.md` "Spec changes" entry per the project non-goals.

...
| `gate-11-firewall-parity` | bash `firewall.sh` agrees with engine `firewall check` across the cross-vault confinement matrix | bash twin verdict diverges from the engine |
...
Gates 05 and 11-firewall-parity are the engine/bash parity invariants: they keep the hot-path bash twins (`verify-ingest.sh`, `firewall.sh`) honest against the TypeScript engine. Gate 13 makes "no RAG" a static CI invariant rather than a runtime hope, and ships a `--self-test` that plants forbidden tokens to prove it fails closed.

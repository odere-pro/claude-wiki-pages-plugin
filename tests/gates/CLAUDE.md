...
| `gate-14-feature-coverage` | tests-as-documentation — every `@test` leads with its FEATURE INDEX label, every top-level `describe` opens with a `Feature:` prefix, the FEATURE INDEX is set-equal to `tests/scripts/*.bats`, and every feature (hook/verb/skill/agent/command) has a documenting test | non-conforming title, stale/orphan INDEX row, or an undocumented feature (`src/core/feature-coverage.ts`, unit-tested by gate-01) |

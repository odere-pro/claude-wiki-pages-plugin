...
The **scaffolding ablation** (ADR-0020) reuses this apparatus to measure what the plugin buys over plain extraction: [`../../scripts/eval-produce-baseline.sh`](../../scripts/eval-produce-baseline.sh) produces the generic-prompt control arm, [`../../scripts/eval-ablation-report.sh`](../../scripts/eval-ablation-report.sh) renders the arms × tiers × cases matrix (a report, never a gate; protocol-violating baseline output renders as a labeled `unscorable` cell). Evidence under `runs/<tier>/qwen3-coder-30b-baseline/` (canonical) and `runs/<tier>/claude-arm/` (supplementary, non-reproducible). The planted `candidate-baseline-shape/` under `extract-basic/` pins rc 1, never rc 2, for frontmatter-less candidates.

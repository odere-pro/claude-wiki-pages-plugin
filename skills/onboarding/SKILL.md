...
1. **Health check.** Run `/claude-wiki-pages:wiki-doctor` (or `bash scripts/doctor.sh`). If anything is red, run it with `--fix` and explain what was repaired. Confirm Bun is present (the engine) — if not, note that hooks still work and point to https://bun.sh.
...
End with a short "what you can do next" map: ingest more sources (`/claude-wiki-pages:wiki`), check health (`/claude-wiki-pages:wiki-doctor`), ask questions (`/claude-wiki-pages:query`), and where rollback lives (`git log` in the vault). Point power users at `/claude-wiki-pages:engine-api` to drive the deterministic engine directly.

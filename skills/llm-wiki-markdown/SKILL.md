...
description: >
  ...
  /claude-wiki-pages:llm-wiki-markdown directly. Reuses the llm-wiki-query
...
This skill is the publish-side counterpart to `/claude-wiki-pages:llm-wiki-query`.
...
- The user invokes `/claude-wiki-pages:llm-wiki-markdown` directly.
...
`/claude-wiki-pages:llm-wiki-query` for that.

Do NOT invoke for synthesis notes (cross-topic analyses live under
`wiki/_synthesis/`) — use `/claude-wiki-pages:llm-wiki-synthesize`.

## Reading contract

Same order and scope as `/claude-wiki-pages:llm-wiki-query`:
...
---
*Generated by `/claude-wiki-pages:llm-wiki-markdown` on <YYYY-MM-DD>. Sources: [[Source Page A]], [[Source Page B]].*
```

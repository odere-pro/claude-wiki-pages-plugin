...
- **`/claude-wiki-pages:wiki` detects and autofixes graph issues.** The orchestrator now probes `graph_health` on every run and routes a drifted-but-idle vault — the common case right after a **plugin update** — to the polish agent's new self-heal sequence (ghost-heal → disentangle → orphan-reconnect → graph colors → index/MOC), then reports the health score. So a plain `/claude-wiki-pages:wiki` with no arguments detects and fixes the graph without the user naming the problem.
...
- **Universal, not dogfood-shaped — the graph machinery now works on ANY project (1.2.0).** Several scripts assumed the plugin's own vault and silently misbehaved elsewhere: `graph-quality.ts` and `disentangle-links.ts` hardcoded a 7-folder topic list (so a foreign vault scored `Cn=0` and its folder-note spine links got demoted on `--apply`); `heal-orphan-sources.ts` hardcoded both the dogfood git path and a 7-topic title map; the `fill-gaps` workflow hardcoded the plugin's 7 topics, repo file paths, and hub titles (so on any other repo nothing staged and the hub gate could never pass). All of these now **derive topics from the vault's own `wiki/` folders** through one shared helper, `src/core/topics.ts` (`deriveTopics`), so there is a single source of truth and no second hardcoded list can drift.

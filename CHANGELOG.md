...
## [1.3.0](https://github.com/odere-pro/claude-wiki-pages-plugin/compare/v1.2.1...v1.3.0) (2026-06-26)
...
- **Self-health estimation (`scripts/health-score.{ts,sh}`).** A deterministic 0–100 score + letter grade aggregating signals the engine already emits (dangling, connectivity, cluster shape, structural verify) plus read-only `--check` drift probes for stale `.obsidian` config and ghost links — surfaced as `needsHeal` + concrete `issues[]`. No new measurement, NO-RAG, read-only.
- **`/claude-wiki-pages:wiki` detects and autofixes graph issues.** The orchestrator now probes `graph_health` on every run and routes a drifted-but-idle vault — the common case right after a **plugin update** — to the polish agent's new self-heal sequence (ghost-heal → strict-tree-reduce → orphan-reconnect → graph colors → index/MOC), then reports the health score. So a plain `/claude-wiki-pages:wiki` with no arguments detects and fixes the graph without the user naming the problem.
- **ROOT hub node.** `wiki/index.md` is now drawn as the graph's ROOT (un-hidden from the island filter, given a distinct white color group, aliased `ROOT`) so every topic island visibly hangs off one findable entry point.
- **Catalog-aware enrichment in `fill-gaps`.** When a structured catalog (e.g. a glossary `.json` with an `entities[]`/families) exists in `raw/`, the workflow expands each missing family into one grounded family page drafted into `_proposed/` for a single `review` promote — making a project's described taxonomy (patterns, anti-patterns, code-smells, risks, principles) visible as nodes.

- **`voice` skill — the house writing voice, shipped as a teaching skill.** New `skills/voice/SKILL.md` (`disable-model-invocation: true`) defines two registers — explanatory (Karpathy-simple, for what/why prose) and engineer (precise, for contracts/design/ADRs) — an LLM-artifact blocklist, the vocabulary and register-separation rules the doc gate enforces, and how to author a wiki page. The note-authoring path (`skills/ingest`, `skills/draft`, `skills/ingest-pipeline`, the ingest and extract-worker agents) now points at it, so plugin-authored pages open with a plain one-line definition and stay out of marketing register. Brings the skill count to 26 (6 agent-teaching). A reusable `.claude/workflows/voice-pass.mjs` (built on the wiki-brainstorm personas) defines and applies the voice across the docs.

- **Host-project intake — "generate a vault for the project" now ingests the repo's docs (ADR-0024).** On a fresh vault the onboarding/init path offers an explicit choice — ingest the whole project's docs (README, `docs/`, ADRs/RFCs; never source code) vs. start with the bundled sample — and on an existing vault an explicit "ingest the whole project / wiki all my docs" prompt is a new orchestrator intent route that wires the project then ingests. Reuses the existing `scripts/wire-source.sh` staging.
...
- **Distribution moved to the `odere-pro/claude-software-3-0-marketplace` registry; the repo ships no self-marketplace.** `.claude-plugin/marketplace.json` is removed — `claude-wiki-pages` is now installed via the shared `odere-pro` marketplace repo (which lists this repo as a GitHub source), not its own in-repo marketplace. Install docs point at `/plugin marketplace add odere-pro/claude-software-3-0-marketplace`; the manifest gate (`validate-manifests.sh`) already treats a missing `marketplace.json` as intentional. Local-clone development uses a contributor-created throwaway `marketplace.json` (see `docs/install.md`).
...
- **Universal, not dogfood-shaped — the graph machinery now works on ANY project (1.2.0).** Several scripts assumed the plugin's own vault and silently misbehaved elsewhere: `graph-quality.ts` and the link reducer hardcoded a 7-folder topic list (so a foreign vault scored `Cn=0` and its folder-note spine links got demoted on `--apply`); `heal-orphan-sources.ts` hardcoded both the dogfood git path and a 7-topic title map; the `fill-gaps` workflow hardcoded the plugin's 7 topics, repo file paths, and hub titles (so on any other repo nothing staged and the hub gate could never pass). All of these now **derive topics from the vault's own `wiki/` folders** through one shared helper, `src/core/topics.ts` (`deriveTopics`), so there is a single source of truth and no second hardcoded list can drift.
- **Escaped-pipe ghost twins (the duplicate grey nodes).** A wikilink cited inside a markdown table escapes its pipe as `\|` (e.g. `_sources/manifest.md`'s `[[pattern-scanner\|pattern-scanner]]`); the resolver kept the trailing `\`, producing dangling ghost twins of real pages. `normaliseTarget` (and its gate-05 twin) now strip the escaped pipe, so table-cited links resolve. Regression-tested in `link-resolver.test.ts` and `wikilink-check.test.ts`.
- **Stale `docs/vault-example/` references repointed (follow-up to ADR-0029 / PR #40).** The glossary, `validate-docs.sh` comments, the wiki-brainstorm `TEAM-BRIEF`, and `docs/automation.md` pointed at the deleted `docs/vault-example/` path; they now cite the schema authority `skills/init/template/CLAUDE.md` and the golden fixture `tests/fixtures/reference-vault`. Description-text only — no canonical term renamed.
- **Nested/wired raw sources were never ingested (ADR-0024).** Wired project docs land nested at `raw/wired/<name>/<relpath>`, but the ingest agent enumerated `raw/*.md` top-level only, so it ingested zero of them even though the orchestrator counted them as pending. The ingest agent now reads pending sources from `engine.sh backlog --json` (`.pendingRaw[]`, recursive, `assets/`-excluded, log-deduped) with a recursive `find` fallback; `resolve-vault.sh`'s `vault_list --status` count drops `-maxdepth 1`. Locked by a nested-source backlog test.
...
- **`disentangle-links.{sh,ts}` (the topic-local linking pass) and its bats.** Retired in favour of `strict-tree-reduce`, whose keep predicate is a strict subset; `src/core/link-demote.ts` is now its sole consumer. The unused `Status` type export (`src/core/report.ts`) was dropped in the same dead-code pass.
